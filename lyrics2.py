...
def main(stdscr):
    ...
    while True:
        ...
        height, width = stdscr.getmaxyx()
        max_scroll_lines_display = height - 3

        if audio_file != current_audio_file:
            current_audio_file = audio_file
            scroll_offset = 0  # Reset scroll offset on track change
            stdscr.clear()
        ...
        if key == curses.KEY_UP:
            ...
            max_possible = max(0, len(lyrics) - max_scroll_lines_display)
            scroll_offset = min(scroll_offset + 1, max_possible)
        elif key == ord('q'):
            break

        # Adjust timeout dynamically
        stdscr.timeout(500 if key == -1 else 50)
